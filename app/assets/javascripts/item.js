...
$(document).ready(function () {                                                 //HTML=DOMの読み込みが終わったらfunction()中の処理を実行
  ...
  $(".file").on('change', function(){                                       
     var fileprop = $(this).prop('files')[0],                               //アップロードするファイルを選択
         ...
         view_box = $(this).parent('.sellUpload__imageview');  //先祖要素であるクラスを指定
    ...
    //挿入するHTMLファイル
    //----------------------------------------------------------------------
    var imgHTML =  '<div class="sellUpload__drop__box__preview"> <img alt="" class="uploadImg"> <a class="imgDel"> 画像を削除する </a> </div>';
    view_box.append(imgHTML);                                               //'.sellUpload__imageview'クラスに挿入
    //----------------------------------------------------------------------
    //画像の読み込み
    //----------------------------------------------------------------------
  ...
  //画像の削除
  //------------------------------------------------------------------------
  function imgDel(target){
    target.find("a.imgDel").on('click',function(){
      var self = $(this)
          parentBox = self.parent().parent().parent();
          parentBox.find('input[type=file]').val('');
          parentBox.find('.sellUpload__drop__box__preview').remove();
    });
  }
});
